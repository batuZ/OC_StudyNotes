...
/*  参考： https://www.cnblogs.com/FBiOSBlog/p/5822325.html    */
...
+(SqliteTools*)shareinstance{
    static SqliteTools *tool = nil;
    static dispatch_once_t oneToken;
    dispatch_once(&oneToken, ^{
        tool=[[SqliteTools alloc] init];
    });
    return tool;
}
...
- (BOOL)insertModel:(Model *)model{
    NSString *sqlStr = [NSString stringWithFormat:@"insert into test (name,age,sex) values ('%@','%ld','%@')",model.name,(long)model.age,model.sex];
    //执行sql语句
    int result = sqlite3_exec(_dbPoint, [sqlStr UTF8String], nil, nil, nil);
    if (result == SQLITE_OK) {
...
#pragma mark - delete
//删除表中的内容
- (BOOL)deletedateModel:(Model *)model{
...
- (BOOL)updateModel:(Model *)model{
    NSString *sqlStr= [NSString stringWithFormat:@"update test set sex='%@',age=%ld where name='%@'",model.sex,(long)model.age,model.name];
    //执行sql语句
    int result = sqlite3_exec(_dbPoint, [sqlStr UTF8String], nil, nil, nil);
    if (result == SQLITE_OK) {
        NSLog(@"修改成功");
